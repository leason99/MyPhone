...
public class ContactEditorFragment extends Activity {

    private ImageView back, edit, ok;
    private ImageView  addSipAddress, contactPicture;
    ...
    byte[] photoToAdd;
    public static  ContactEditorFragment instance;
    public static  ContactEditorFragment instance() {
        if (instance != null)
            return instance;
        throw new RuntimeException("contacteditor not instantiated yet");
    }
       ...
       super.onCreate(savedInstanceState);
        ...
        if (bundle != null) {
            if (bundle.getSerializable("Contact") != null) {
                ...
                if (bundle.getString("NewSipAdress") != null) {
                    newSipOrNumberToAdd = bundle.getString("NewSipAdress");
                }

            } else if (bundle.getString("NewSipAdress") != null) {
                newSipOrNumberToAdd = bundle.getString("NewSipAdress");
                isNewContact = true;
            }
        }
        ...
        if (contact != null && contact.getPhotoUri() != null) {
            InputStream input = Compatibility.getContactPictureInputStream(getContentResolver(), contact.getID());
        ...
        addSipAddress = (ImageView) findViewById(R.id.add_address_field);
...
*/
    private LinearLayout initSipAddressFields(final Contact contact) {
        LinearLayout controls = (LinearLayout)findViewById(R.id.controls_sip_address);
        controls.removeAllViews();
    ...
    class NewOrUpdatedNumberOrAddress {
        ...
        private void addNewNumber() {
            ...
            if (isNewContact) {
                if (isSipAddress) {
                    ...
                    Compatibility.addSipAddressToContact(instance(), ops, newNumberOrAddress);
                } else {
                    ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
                ...
                String rawContactId = contactsManager.findRawContactID(getContentResolver(),String.valueOf(contactID));
                if (isSipAddress) {
                    if (newNumberOrAddress.startsWith("sip:"))
                        newNumberOrAddress = newNumberOrAddress.substring(4);
                    if(!newNumberOrAddress.contains("@")) {
                    ...
                    Compatibility.addSipAddressToContact(instance(), ops, newNumberOrAddress, rawContactId);
                    if (getResources().getBoolean(R.bool.use_linphone_tag)) {
                        Compatibility.addLinphoneContactTag(instance(), ops, newNumberOrAddress, contactsManager.findRawLinphoneContactID(String.valueOf(contactID)));
                    }
                } else {
                    ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
                                    .withValue(ContactsContract.Data.RAW_CONTACT_ID, rawContactId)
        ...
        private void updateNumber() {
            ...
            if (isSipAddress) {
                ...
                if (getResources().getBoolean(R.bool.use_linphone_tag)) {
                    Compatibility.updateLinphoneContactTag(instance(), ops, newNumberOrAddress, oldNumberOrAddress, contactsManager.findRawLinphoneContactID(String.valueOf(contactID)));
                }
            } else {
                String select = ContactsContract.Data.CONTACT_ID + "=? AND "
                        + ContactsContract.Data.MIMETYPE + "='" + ContactsContract.CommonDataKinds.Phone.CONTENT_ITEM_TYPE +  "' AND "
